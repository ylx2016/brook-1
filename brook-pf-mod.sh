...
Download_brook(){
    ...
    cd ${file}
    if [[ ${bit} == "x86_64" ]]; then
        wget  -N "https://github.com/txthinking/brook/releases/download/${brook_new_ver}/brook_linux_amd64"
		mv brook_linux_amd64 brook
    else
        wget  -N "https://github.com/txthinking/brook/releases/download/${brook_new_ver}/brook_linux_386"
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget  https://raw.githubusercontent.com/monret/brook/master/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget  https://raw.githubusercontent.com/monret/brook/master/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
